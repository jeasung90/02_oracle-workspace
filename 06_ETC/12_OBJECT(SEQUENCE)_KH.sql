/*
    ...
    점수값을 순차적으로 일정값씩 증가시키면서 생성해줌 (기본적으로는 1씩 증가)
...
/*
    ...
    [상세 표현식]
    CREATE SEQUENCE 시퀀스명
    [START WITH 시작숫자]     -- 처음 발생시키는 시작값 지정 (기본값 1)
    [INCREMENT BY 숫자]      -- 몇씩 증가시킬건지 (기본값 1)
    [MAXVALUE 숫자]          -- 최대값 지정 (기본값 겁나큼...)
    [MINVALUE 숫지]          -- 최소값 지정 (기본값 1)
    [CYCLE | NOCYCLE]        -- 값 순환 여부 지정 (기본값 NOCYCLE) => 최대값을 찍고 처음부터 다시 돌아와서 시작할 수 있게함
    [NOCASHE | CASHE 바이트크기] -- 캐시메모리 할당 (기본값 CACHE 20)
...
-- 왜? 마지막으로 성공적으로 수행된 NEXTVAL 값이기 대문!!
